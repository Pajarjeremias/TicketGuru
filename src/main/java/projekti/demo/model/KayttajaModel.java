package projekti.demo.model;

import jakarta.persistence.Entity;
import jakarta.persistence.GeneratedValue;
import jakarta.persistence.GenerationType;
import jakarta.persistence.Id;
import jakarta.persistence.JoinColumn;
import jakarta.persistence.ManyToOne;
import jakarta.persistence.Table;
import jakarta.validation.constraints.NotEmpty;
import jakarta.validation.constraints.NotNull;
import jakarta.validation.constraints.Size;


public class KayttajaModel {

  @Size(max = 60, message = "Maksimipituus 60 merkkiä")
  private String salasana_hash;

  @Size(max = 100, message = "Maksimipituus 100 merkkiä")
  private String etunimi;

  @Size(max = 100, message = "Maksimipituus 100 merkkiä")
  private String sukunimi;

  @Size(max = 20, message = "Maksimipituus 20 merkkiä")
  private String puh_nro;

  @Size(max = 100, message = "Maksimipituus 100 merkkiä")
  private String email;

  @Size(max = 100, message = "Maksimipituus 100 merkkiä")
  private String katuosoite;

  private Postitoimipaikka postinumero;

  // Constructorit
 

  public KayttajaModel() {
}

  public KayttajaModel(@Size(max = 60, message = "Maksimipituus 60 merkkiä") String salasana_hash,
        @Size(max = 100, message = "Maksimipituus 100 merkkiä") String etunimi,
        @Size(max = 100, message = "Maksimipituus 100 merkkiä") String sukunimi,
        @Size(max = 20, message = "Maksimipituus 20 merkkiä") String puh_nro,
        @Size(max = 100, message = "Maksimipituus 100 merkkiä") String email,
        @Size(max = 100, message = "Maksimipituus 100 merkkiä") String katuosoite, Postitoimipaikka postinumero) {
    this.salasana_hash = salasana_hash;
    this.etunimi = etunimi;
    this.sukunimi = sukunimi;
    this.puh_nro = puh_nro;
    this.email = email;
    this.katuosoite = katuosoite;
    this.postinumero = postinumero;
  }

  public KayttajaModel(@Size(max = 100, message = "Maksimipituus 100 merkkiä") String katuosoite,
        Postitoimipaikka postinumero) {
    this.katuosoite = katuosoite;
    this.postinumero = postinumero;
  }

  public KayttajaModel(@Size(max = 60, message = "Maksimipituus 60 merkkiä") String salasana_hash) {
    this.salasana_hash = salasana_hash;
  }

  public String getSalasana_hash() {
    return salasana_hash;
  }

  public void setSalasana_hash(String salasana_hash) {
    this.salasana_hash = salasana_hash;
  }

  public String getEtunimi() {
    return etunimi;
  }

  public void setEtunimi(String etunimi) {
    this.etunimi = etunimi;
  }

  public String getSukunimi() {
    return sukunimi;
  }

  public void setSukunimi(String sukunimi) {
    this.sukunimi = sukunimi;
  }

  public String getPuh_nro() {
    return puh_nro;
  }

  public void setPuh_nro(String puh_nro) {
    this.puh_nro = puh_nro;
  }

  public String getEmail() {
    return email;
  }

  public void setEmail(String email) {
    this.email = email;
  }

  public String getKatuosoite() {
    return katuosoite;
  }

  public void setKatuosoite(String katuosoite) {
    this.katuosoite = katuosoite;
  }

  public Postitoimipaikka getPostinumero() {
    return postinumero;
  }

  public void setPostinumero(Postitoimipaikka postinumero) {
    this.postinumero = postinumero;
  }









  
}
